...
#
# Writes: dist/feareffect2retrohelix-<VERSION>-<artifact-tag>.zip
set -euo pipefail
...
exec bash "${PACKAGER}" \
  ...
  --zip-prefix feareffect2retrohelix \
  --exe-name Fear_Effect_2_Retro_Helix \
  --omit-openbios \
  --display-name "Fear Effect 2: Retro Helix" \
  --recompiler-build "${RECOMPILER_BUILD}" \
  --version-env RELEASE_VERSION \
  --disc-hint "your legally owned Fear Effect 2: Retro Helix (USA) discs" \
  --runtime-dir mods \
